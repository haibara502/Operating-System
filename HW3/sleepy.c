...
ssize_t sleepy_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (mutex_lock_killable(&dev->sleepy_mutex)) //If it is not readable
	  return -EINTR;
	
	/* YOUR CODE HERE */
	minor = (int)iminor(filp -> f_path.dentry -> d_inode);
	printk("SLEEPY_READ DEVICE DEVICE (%d): Process is waking everyone up.\n", minor);

	wake_up_interruptible(&dev -> wait_queue[minor]);
	/* END YOUR CODE */
...
ssize_t 
sleepy_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	struct sleepy_dev *dev = (struct sleepy_dev *)filp->private_data;
	int time_number = 0;
	int minor = 0;
	ssize_t remaining_time = 0;
	...
	if (copy_from_user(&time_number, buf, count) != 0)
		return -EIO;
	...
	unsigned long time_length = time_number * 1000;
	unsigned long end_time = current_time_stamp + time_length;

	mutex_unlock(&dev -> sleepy_mutex);	
	if (wait_event_interruptible_timeout(dev -> wait_queue[minor], 1, time_length))
	{
		remaining_time = (end_time - jiffies) / 1000;
		printk("SLEEPY_WRITE DEVICE (%d): remainnig = %zd \n", minor, remaining_time);
	}
	if (mutex_lock_killable(&dev -> sleepy_mutex))
		return -EINTR;
